'use client';

import { useState, useEffect, useTransition } from 'react';
import {
    DocumentArrowDownIcon,
    ExclamationTriangleIcon,
    ChevronDownIcon,
    ChevronUpIcon,
    FolderOpenIcon,
    UserCircleIcon,
    CalendarDaysIcon,
    DocumentTextIcon,
    CheckCircleIcon,
    UsersIcon,
    BuildingLibraryIcon,
    ClipboardDocumentCheckIcon,
    SparklesIcon,
} from '@heroicons/react/24/outline';
import { cn } from '@/lib/utils';
import {
    ChatRecordsStatus,
    DataPanelProps,
    Message,
    FieldProps,
    SectionProps,
    StructuredData
} from '@/types/types';
import { formatTitle, formatDate, formatEstateSize, yesNo } from '@/lib/utils';
import { updateRecordStatus } from '@/app/dashboard/actions';

function asStructured(raw: Record<string, unknown> | null | undefined): StructuredData | null {
    if (!raw) return null;
    return raw as unknown as StructuredData;
}

function Field({ label, value, className }: FieldProps) {
    return (
        <div className={cn('space-y-0.5', className)}>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">
                {label}
            </p>
            <p className="text-sm leading-snug text-zinc-800 dark:text-zinc-200">
                {value || '—'}
            </p>
        </div>
    );
}

function Section({ title, icon: Icon, children, cols = 2 }: SectionProps) {
    return (
        <div className="space-y-4 rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
            <div className="flex items-center gap-2">
                <Icon className="h-4 w-4 shrink-0 text-zinc-400 dark:text-zinc-500" />
                <h3 className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">
                    {title}
                </h3>
            </div>
            <div className={cn('grid gap-x-6 gap-y-4', cols === 2 ? 'grid-cols-2' : 'grid-cols-1')}>
                {children}
            </div>
        </div>
    );
}

function StatusBadge({ status }: { status: ChatRecordsStatus }) {
    return (
        <span className={cn(
            'inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium',
            status === 'new'
                ? 'bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300'
                : 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400',
        )}>
            {status !== 'new' && <CheckCircleIcon className="h-3 w-3" />}
            {formatTitle(status)}
        </span>
    );
}

function EmptyState({ userName }: { userName: string | null }) {
    return (
        <div className="flex flex-1 flex-col items-center justify-center gap-3 bg-zinc-50 px-8 text-center dark:bg-zinc-950">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800">
                <FolderOpenIcon className="h-6 w-6 text-zinc-400 dark:text-zinc-500" />
            </div>
            {userName && (
                <p className="text-sm font-medium text-zinc-500 dark:text-zinc-400">
                    Welcome, {userName}
                </p>
            )}
            <p className="text-sm text-zinc-400 dark:text-zinc-500">
                Select a client to view their onboarding record.
            </p>
        </div>
    );
}

function TranscriptSection({ transcript, open, onToggle }: {
    transcript: Message[] | null;
    open: boolean;
    onToggle: () => void;
}) {
    if (!transcript?.length) return null;
    return (
        <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
            <button
                type="button"
                onClick={onToggle}
                className="flex w-full items-center justify-between px-5 py-4 text-left transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50"
            >
                <div className="flex items-center gap-2">
                    <DocumentTextIcon className="h-4 w-4 text-zinc-400 dark:text-zinc-500" />
                    <span className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">
                        Full Transcript
                    </span>
                    <span className="text-xs text-zinc-400 dark:text-zinc-500">
                        ({transcript.length} messages)
                    </span>
                </div>
                {open
                    ? <ChevronUpIcon className="h-4 w-4 text-zinc-400" />
                    : <ChevronDownIcon className="h-4 w-4 text-zinc-400" />
                }
            </button>
            {open && (
                <div className="max-h-120 space-y-4 overflow-y-auto border-t border-zinc-100 px-5 py-4 dark:border-zinc-800">
                    {transcript.map((msg, i) => (
                        <div key={i} className={cn('flex gap-2.5', msg.role === 'user' ? 'flex-row-reverse' : 'flex-row')}>
                            <div className={cn(
                                'flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-bold',
                                msg.role === 'user'
                                    ? 'bg-zinc-200 text-zinc-600 dark:bg-zinc-700 dark:text-zinc-300'
                                    : 'bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900',
                            )}>
                                {msg.role === 'user' ? 'C' : 'H'}
                            </div>
                            <div className={cn(
                                'max-w-[85%] rounded-xl px-3.5 py-2.5 text-sm leading-relaxed',
                                msg.role === 'user'
                                    ? 'bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200'
                                    : 'border border-zinc-200 bg-zinc-50 text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800/50 dark:text-zinc-300',
                            )}>
                                {typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content)}
                            </div>
                        </div>
                    ))}
                </div>
            )}
        </div>
    );
}

export default function DataPanel({ record, userName }: DataPanelProps) {
    const [transcriptOpen, setTranscriptOpen] = useState(false);
    const [recordStatus, setRecordStatus] = useState<ChatRecordsStatus>(record?.status ?? 'new');
    const [isPending, startTransition] = useTransition();

    useEffect(() => {
        // eslint-disable-next-line react-hooks/set-state-in-effect
        if (record) setRecordStatus(record.status);
    }, [record]);

    const handleStatusToggle = () => {
        const next: ChatRecordsStatus = recordStatus === 'new' ? 'reviewed' : 'new';
        setRecordStatus(next);
        startTransition(async () => {
            const result = await updateRecordStatus(record!.id, next);
            if (!result.success) {
                setRecordStatus(recordStatus);
                console.error('[DataPanel] Status update failed:', result.error);
            }
        });
    };

    if (!record) return <EmptyState userName={userName} />;

    const structured = asStructured(record.structuredData as Record<string, unknown> | null);

    if (!structured) {
        return (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 bg-zinc-50 px-8 text-center dark:bg-zinc-950">
                <p className="text-sm text-zinc-500 dark:text-zinc-400">Intake data is still processing.</p>
                <p className="text-xs text-zinc-400 dark:text-zinc-500">This may take a moment.</p>
            </div>
        );
    }

    const {
        client_identification,
        family_situation,
        assets_overview,
        existing_documents,
        planning_goals,
        scheduling_preference,
        session_metadata,
    } = structured;

    const hasFlags = session_metadata.complexity_flags && session_metadata.complexity_flags.length > 0;

    return (
        <div className="flex h-full flex-col overflow-y-auto bg-zinc-50 dark:bg-zinc-950">
            {/* Sticky header */}
            <div className="sticky top-0 z-10 border-b border-zinc-200 bg-zinc-50/95 px-6 py-4 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/95 supports-backdrop-filter:bg-zinc-50/80 dark:supports-backdrop-filter:bg-zinc-950/80">
                <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0 space-y-0.5">
                        <h2 className="truncate text-xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">
                            {record.clientName ?? client_identification.full_name}
                        </h2>
                        <p className="text-sm text-zinc-500 dark:text-zinc-400">
                            Completed {formatDate(record.completedAt ?? record.createdAt)}
                        </p>
                    </div>
                    <div className="flex shrink-0 items-center gap-3">
                        <StatusBadge status={recordStatus} />
                        <button
                            type="button"
                            onClick={handleStatusToggle}
                            disabled={isPending}
                            className="text-xs text-zinc-500 transition-colors hover:text-zinc-800 disabled:opacity-40 dark:text-zinc-400 dark:hover:text-zinc-200"
                        >
                            {recordStatus === 'new' ? 'Mark as reviewed' : 'Mark as new'}
                        </button>
                        {record.pdfUrl ? (
                            <a
                                href={record.pdfUrl}
                                download
                                className="flex items-center gap-1.5 rounded-lg bg-zinc-900 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-zinc-700 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-300"
                            >
                                <DocumentArrowDownIcon className="h-4 w-4" />
                                Download PDF
                            </a>
                        ) : (
                            <span className="text-xs italic text-zinc-400 dark:text-zinc-500">PDF pending</span>
                        )}
                    </div>
                </div>
            </div>
            <div className="flex flex-col gap-4 p-6">
                <div className="space-y-3 rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
                    <h3 className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">
                        Case Summary
                    </h3>
                    <p className="text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">
                        {session_metadata.conversation_summary}
                    </p>
                    {hasFlags && (
                        <div className="space-y-2 border-t border-zinc-100 pt-3 dark:border-zinc-800">
                            {session_metadata.complexity_flags!.map((flag, i) => (
                                <div key={i} className="flex items-start gap-2">
                                    <ExclamationTriangleIcon className="mt-0.5 h-4 w-4 shrink-0 text-amber-500 dark:text-amber-400" />
                                    <p className="text-xs text-zinc-600 dark:text-zinc-400">
                                        <span className="font-semibold text-zinc-700 dark:text-zinc-300">{flag.topic}</span>
                                        {' — '}{flag.reason}
                                    </p>
                                </div>
                            ))}
                        </div>
                    )}
                </div>

                {/* Main data grid */}
                <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                    {family_situation && (
                        <Section title="Family Situation" icon={UsersIcon}>
                            <Field label="Marital status" value={formatTitle(family_situation.marital_status)} />
                            <Field label="Has children" value={yesNo(family_situation.has_children)} />
                            {family_situation.has_children && (
                                <Field label="Number of children" value={family_situation.number_of_children?.toString() ?? '—'} />
                            )}
                            <Field label="Has dependents" value={yesNo(family_situation.has_dependents)} />
                            {family_situation.dependent_notes && (
                                <Field label="Dependent notes" value={family_situation.dependent_notes} className="col-span-2" />
                            )}
                        </Section>
                    )}

                    {assets_overview && (
                        <Section title="Assets Overview" icon={BuildingLibraryIcon}>
                            <Field label="Estimated estate size" value={formatEstateSize(assets_overview.estimated_estate_size)} />
                            <Field label="Real estate" value={yesNo(assets_overview.has_real_estate)} />
                            <Field label="Business interests" value={yesNo(assets_overview.has_business_interests)} />
                            <Field label="Retirement accounts" value={yesNo(assets_overview.has_retirement_accounts)} />
                            <Field label="Life insurance" value={yesNo(assets_overview.has_life_insurance)} />
                            {assets_overview.asset_notes && (
                                <Field label="Notes" value={assets_overview.asset_notes} className="col-span-2" />
                            )}
                        </Section>
                    )}

                    {existing_documents && (
                        <Section title="Existing Documents" icon={ClipboardDocumentCheckIcon}>
                            <Field label="Existing will" value={yesNo(existing_documents.has_existing_will)} />
                            <Field label="Trust" value={yesNo(existing_documents.has_trust)} />
                            <Field label="Power of attorney" value={yesNo(existing_documents.has_power_of_attorney)} />
                            <Field label="Healthcare directive" value={yesNo(existing_documents.has_healthcare_directive)} />
                            {existing_documents.documents_notes && (
                                <Field label="Notes" value={existing_documents.documents_notes} className="col-span-2" />
                            )}
                        </Section>
                    )}

                    {planning_goals && (
                        <Section title="Planning Goals" icon={SparklesIcon}>
                            <Field label="Avoid probate" value={yesNo(planning_goals.avoid_probate)} />
                            <Field label="Minimize taxes" value={yesNo(planning_goals.minimize_taxes)} />
                            <Field label="Charitable giving" value={yesNo(planning_goals.charitable_giving)} />
                            <Field label="Business succession" value={yesNo(planning_goals.business_succession)} />
                            {planning_goals.goals_notes && (
                                <Field label="Notes" value={planning_goals.goals_notes} className="col-span-2" />
                            )}
                        </Section>
                    )}
                </div>

                {/* Contact info and scheduling preference */}
                <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                    <Section title="Client Contact" icon={UserCircleIcon}>
                        <Field label="Name" value={client_identification.full_name} />
                        <Field label="Phone" value={client_identification.phone ?? '—'} />
                        <Field label="Email" value={client_identification.email ?? '—'} />
                    </Section>

                    <Section title="Scheduling Preference" icon={CalendarDaysIcon}>
                        <Field label="Format" value={formatTitle(scheduling_preference.preferred_format)} />
                        <Field label="Urgency" value={formatTitle(scheduling_preference.urgency_to_consult)} />
                        {scheduling_preference.preferred_times && (
                            <Field label="Preferred times" value={scheduling_preference.preferred_times} className="col-span-2" />
                        )}
                        {scheduling_preference.availability_notes && (
                            <Field label="Availability notes" value={scheduling_preference.availability_notes} className="col-span-2" />
                        )}
                    </Section>
                </div>

                {session_metadata.additional_notes && (
                    <div className="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
                        <h3 className="mb-3 text-[10px] font-semibold uppercase tracking-widest text-zinc-500 dark:text-zinc-400">
                            Additional Notes
                        </h3>
                        <p className="text-sm leading-relaxed text-zinc-700 dark:text-zinc-300">
                            {session_metadata.additional_notes}
                        </p>
                    </div>
                )}
                <TranscriptSection
                    transcript={record.transcript as Message[] | null}
                    open={transcriptOpen}
                    onToggle={() => setTranscriptOpen((v) => !v)}
                />
            </div>
        </div>
    );
}